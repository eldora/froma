...
signed portBASE_TYPE xTaskGenericCreate( portBASE_TYPE xProcessor, pdTASK_CODE pxTaskCode, const signed char * const pcName, unsigned short usStackDepth, void *pvParameters, unsigned portBASE_TYPE uxPriority, xTaskHandle *pxCreatedTask, portSTACK_TYPE *puxStackBuffer, const xMemoryRegion * const xRegions )
{
...
void vTaskStartScheduler( void )
{
	...
	xReturn = xTaskCreate( prvIdleTask, ( signed char * ) "IDLE1", tskIDLE_STACK_SIZE, 
			( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), ( xTaskHandle * ) NULL );
	xReturn = xTaskCreate( prvIdleTask, ( signed char * ) "IDLE2", tskIDLE_STACK_SIZE, 
			( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), ( xTaskHandle * ) NULL );
...
/*-----------------------------------------------------------*/
// 현재 태스크를 DelayedList에 넣고 xTimeToWake 뒤에 깨어난다?
...
/*-----------------------------------------------------------*/
// 새로운 TCB를 정의하기 위해 heap영역에 TCB구조체 크기와 스택을 할당받아 주소값을 리턴
...
#if ( portCRITICAL_NESTING_IN_TCB == 1 )
// 인터럽트를 비 활성화 시키고, uxCriticalNesting 값을 증가 시킴
...
#if ( portCRITICAL_NESTING_IN_TCB == 1 )
// 인터럽트를 활성화 시키고 uxCriticalNesting 값을 감소 시킴
